...
export const FeedCard = ({ data, index }) => {
  ...
  useEffect(() => {
    let c1 = data.like_reaction
      ? parseInt(data.like_reaction) !== 0
        ? parseInt(data.like_reaction)
        : 0
      : 0;
    let c2 = data.support_reaction
      ? parseInt(data.support_reaction) !== 0
        ? parseInt(data.support_reaction)
        : 0
      : 0;
    let c3 = data.love_reaction
      ? parseInt(data.love_reaction) !== 0
        ? parseInt(data.love_reaction)
        : 0
      : 0;
  ...
  const handleSuppoprt = async (event) => {
    ...
    if (result.payload !== undefined) {
      ...
      let c2 = data.support_reaction
        ? parseInt(data.support_reaction) !== 0
          ? parseInt(data.support_reaction) + 1
          : 1
        : 1;
      let c3 = data.love_reaction
        ? parseInt(data.love_reaction) !== 0
      ...
      <CardActions disableSpacing>
            ...
            >
              {reaction.img ? (
                <>
                  ...
                  >
                    {reaction.description ? reaction.description : "Like"}
                  </span>
                </>
              ) : (
        ...
        <IconButton aria-label="share">
          ...
          >
            ...
            <span
              style={{ fontSize: 13, marginTop: 3, display: "flex", justifyContent: "center" }}
